...
struct ScrollViewWithRefresh<Content: View>: View {
    ...
    init(_ thresholdHeight: CGFloat = 50, refreshing: Binding<Bool>, action: @escaping () -> (),onScroll: @escaping (CGRect, CGPoint) -> Void = {_,_  in }, @ViewBuilder content: () -> Content) {
        ...
        VStack {
            ScrollView {
                ZStack(alignment: .top) {
                    ...
                    RefreshHeader(height: self.thresholdHeight * 0.8, loading: self.refreshing, frozen: self.frozen, rotation: self.rotation)
